...
## Client ID
- Need to register a "Desktop app" OAuth client on a Samuel Kalu GCP project
...
## Token Lifecycle
- Store at `~/.satanclaw/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `satanclaw_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `satanclaw_cli/models.py` — Add Gemini model catalog
4. `satanclaw_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `satanclaw_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `satanclaw_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Samuel Kalu GCP project with Desktop OAuth client registered
...
- satanclaw-agent Copilot OAuth: `satanclaw_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
