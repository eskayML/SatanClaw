...
from satan_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in satan_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from satan_cli.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "satan_cli.codex_models._fetch_models_from_api",
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from satan_cli.main import _model_flow_openai_codex

    captured = {}

    monkeypatch.setattr(
        "satan_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "satan_cli.auth.resolve_codex_runtime_credentials",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a SatanCLI with minimal mocking."""
    ...
    with (
        ...
        cli = SatanCLI(model=model, **kwargs)
    return cli
