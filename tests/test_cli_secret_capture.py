...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("satanclaw_cli.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_getpass_without_tui():
    ...
    with patch("satanclaw_cli.callbacks.getpass.getpass", return_value="secret-value"), patch(
        "satanclaw_cli.callbacks.save_env_value_secure"
    ) as save_secret:
        save_secret.return_value = {
            "success": True,
            "stored_as": "TENOR_API_KEY",
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("satanclaw_cli.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
        "satanclaw_cli.callbacks._time.monotonic",
        side_effect=[0, 121],
    ):
        result = prompt_for_secret(cli, "TENOR_API_KEY", "Tenor API key")
