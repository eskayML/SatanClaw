"""Tests for acp_adapter.server — SatanClawACPAgent ACP server."""
...
from acp_adapter.server import SatanClawACPAgent, HERMES_VERSION
...
def agent(mock_manager):
    """SatanClawACPAgent backed by a mock session manager."""
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "satanclaw-agent"
...
class TestSlashCommands:
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = SatanClawACPAgent(session_manager=manager)
