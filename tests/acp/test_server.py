"""Tests for acp_adapter.server — SatanACPAgent ACP server."""
...
from acp_adapter.server import SatanACPAgent, HERMES_VERSION
...
def agent(mock_manager):
    """SatanACPAgent backed by a mock session manager."""
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "satan-agent"
...
class TestSlashCommands:
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = SatanACPAgent(session_manager=manager)
