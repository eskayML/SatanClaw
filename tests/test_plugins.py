"""Tests for the SatanClaw plugin system (satanclaw_cli.plugins)."""
...
from satanclaw_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.satanclaw/plugins/ are discovered."""
        plugins_dir = tmp_path / "satanclaw_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "satanclaw_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        plugins_dir = project_dir / ".satanclaw" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under satanclaw_plugins.<name>."""
        plugins_dir = tmp_path / "satanclaw_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "satanclaw_test"))

        # Clean up any prior namespace module
        sys.modules.pop("satanclaw_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "satanclaw_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="satanclaw_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import satanclaw_cli.plugins as plugins_mod
...
# in PluginContext (satanclaw_cli/plugins.py).  The tests referenced _plugin_commands,
