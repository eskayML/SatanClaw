...
class TestParserRegistry:
    ...
    def test_satanclaw_parser_registered(self):
        parsers = list_parsers()
        assert "satanclaw" in parsers
...
# ─── SatanClaw parser tests ────────────────────────────────────────────────

class TestSatanClawParser:
    @pytest.fixture
    def parser(self):
        return get_parser("satanclaw")
...
class TestParseResultContract:
    ...
    @pytest.fixture(params=["satanclaw"])  # Add more as needed
    ...
    def test_tool_calls_are_proper_objects(self, parser):
        ...
        # Use satanclaw format since that's universal
