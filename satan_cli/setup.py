"""
Interactive setup wizard for Satan Agent.
...
Config files are stored in ~/.satan/ for easy access.
"""
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from satan_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from satan_cli.config import get_env_value
...
# display_satan_home imported lazily at call sites (stale-module safety during satan update)

from satan_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Satan Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Satan using environment variables or config commands:")
    print_info("  satan config set model.provider custom")
    print_info("  satan config set model.base_url http://localhost:8080/v1")
    print_info("  satan config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'satan setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from satan_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'satan setup')")


def _print_setup_summary(config: dict, satan_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'satan setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'satan setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'satan setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'satan setup tools' to configure them,"
        )
        from satan_constants import display_satan_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {satan_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('satan setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('satan setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('satan setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('satan setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('satan setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('satan config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('satan config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('satan config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('satan', Colors.GREEN)}              Start chatting")
    print(f"   {color('satan gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('satan doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `satan tools` and `satan setup tools`).
...
def setup_model_provider(config: dict):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``satan model``)
    ...
    provider added to ``satan model`` is automatically available here.
    """
    ...
    # Delegate to the shared satan model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from satan_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: satan model")
    ...
    if _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Satan can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Satan can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Satan can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'satan setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.satan/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'satan setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Satan runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Satan via Telegram/Discord, this is where")
        print_info(
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Satan from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Satan delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://satan-agent.nousresearch.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where Satan delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where Satan delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'satan whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'satan whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://satan-agent.nousresearch.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info(
            "   https://satan-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes"
        )
        print()
        print_info("   Open config in your editor:  satan config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     satan config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: satan gateway install")
            else:
                print_info("  You can install later: satan gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo satan gateway install --system")
                print_info("  Or run in foreground:  satan gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   satan gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `satan setup tools` and `satan tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from satan_cli.config
...
def _offer_openclaw_migration(satan_home: Path) -> bool:
    ...
    print_info("Satan can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_satan", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=satan_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Satan.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      satan setup           — full or quick (auto-detected)
      satan setup model     — just model/provider
      satan setup terminal  — just terminal backend
      satan setup gateway   — just messaging platforms
      satan setup tools     — just tool configuration
      satan setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Satan Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Satan Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Satan Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Satan configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, satan_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'satan setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'satan setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, satan_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(satan_home)
        if migration_ran:
    ...
    print_info(f"Data folder:  {satan_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'satan config edit'")
...
def _run_quick_setup(config: dict, satan_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'satan setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Satan to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'satan setup gateway'.")
