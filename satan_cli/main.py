#!/usr/bin/env python3
"""
Satan CLI - Main entry point.

Usage:
    satan                     # Interactive chat (default)
    satan chat                # Interactive chat
    satan gateway             # Run gateway in foreground
    satan gateway start       # Start gateway as service
    satan gateway stop        # Stop gateway service
    satan gateway status      # Show gateway status
    satan gateway install     # Install gateway service
    satan gateway uninstall   # Uninstall gateway service
    satan setup               # Interactive setup wizard
    satan logout              # Clear stored authentication
    satan status              # Show status of all components
    satan cron                # Manage cron jobs
    satan cron list           # List cron jobs
    satan cron status         # Check if cron scheduler is running
    satan doctor              # Check configuration and dependencies
    satan honcho setup                    # Configure Honcho AI memory integration
    satan honcho status                   # Show Honcho config and connection status
    satan honcho sessions                 # List directory → session name mappings
    satan honcho map <name>               # Map current directory to a session name
    satan honcho peer                     # Show peer names and dialectic settings
    satan honcho peer --user NAME         # Set user peer name
    satan honcho peer --ai NAME           # Set AI peer name
    satan honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    satan honcho mode                     # Show current memory mode
    satan honcho mode [hybrid|honcho|local]  # Set memory mode
    satan honcho tokens                   # Show token budget settings
    satan honcho tokens --context N       # Set session.context() token cap
    satan honcho tokens --dialectic N     # Set dialectic result char cap
    satan honcho identity                 # Show AI peer identity representation
    satan honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    satan honcho migrate                  # Step-by-step migration guide: OpenClaw native → Satan + Honcho
    satan version             Show version
    satan update              Update to latest version
    satan uninstall           Uninstall Satan Agent
    satan acp                 Run as an ACP server for editor integration
    satan sessions browse     Interactive session picker with search

    satan claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (satan tools, satan setup, satan model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'satan {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any satan module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.satan/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check ~/.satan/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".satan" / "active_profile"
    ...
    if profile_name is not None:
        try:
            from satan_cli.profiles import resolve_profile_env
            satan_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent satan from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["HERMES_HOME"] = satan_home
...
load_satan_dotenv(project_env=PROJECT_ROOT / '.env')
...
from satan_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from satan_cli.config import get_env_path, get_satan_home, load_config
    from satan_cli.auth import get_auth_status

    # Determine whether Satan itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_satan_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_satan_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Satan has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Satan to use their tokens.
    if _has_satan_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'satan sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Satan isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  satan setup")
        print()

        from satan_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'satan setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from satan_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from satan_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from satan_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Satan?")
        print()
    ...
    session_dir = get_satan_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: satan gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  satan gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Satan Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  satan gateway")
        ...
        print()
        print("  Or install as a service: satan gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'satan whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``satan model``) and the setup wizard
    ...
    from satan_cli.config import load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from satan_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from satan_cli.config import get_env_value, save_env_value
    ...
    from satan_cli.models import model_ids
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from satan_cli.config import get_env_value, save_config, save_env_value
    from satan_cli.nous_subscription import (
    ...
    from satan_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from satan_cli.auth import _save_model_choice, deactivate_provider
    from satan_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from satan_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `satan model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from satan_cli.auth import _save_model_choice, deactivate_provider
    from satan_cli.config import save_env_value, load_config, save_config
    from satan_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from satan_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from satan_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Satan turns to `copilot --acp`.")
    print("  Satan currently starts its own ACP subprocess for each request.")
    print("  Satan uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from satan_cli.config import get_env_value, save_env_value, load_config, save_config
    from satan_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Satan will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'satan model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Satan CLI with a provider."""
    from satan_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from satan_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from satan_cli.auth_commands import auth_command
    auth_command(args)
...
def cmd_cron(args):
    """Cron job management."""
    from satan_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from satan_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from satan_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from satan_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Satan Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from satan_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Satan Agent."""
    _require_tty("uninstall")
    from satan_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _update_via_zip(args):
    """Update Satan Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/satan-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="satan-update-")
        zip_path = os.path.join(tmp_dir, f"satan-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to satan-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"satan-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("satan-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Satan behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Satan can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Satan couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Satan couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Satan behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `satan update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/satan-agent.git",
    "git@github.com:NousResearch/satan-agent.git",
    "https://github.com/NousResearch/satan-agent",
    "git@github.com:NousResearch/satan-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/satan-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_satan_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from satan_constants import get_satan_home
        (get_satan_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Satan repository.")
        print("  This means you may miss updates from NousResearch/satan-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/satan-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/satan-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``satan update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".satan"
    homes.append(default_home)
    # Named profiles under ~/.satan/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "satan-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Satan Agent to the latest version."""
    import shutil
    from satan_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Satan Agent")
        return
    
    print("⚕ Updating Satan Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/satan-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_satan_home added to satan_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload satan_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_satan_home() added since the last release.
        try:
            import importlib
            import satan_constants as _hc
        ...
        try:
            from satan_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                ...
                print("    Run 'satan config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'satan config migrate' later to configure.")
        else:
        ...
        try:
            ...
            # Also check for a system-level service (satan gateway install --system).
            ...
            if is_macos():
                try:
                    from satan_cli.gateway import get_launchd_label
            ...
            if existing_pid or has_systemd_service or has_system_service or has_launchd_service:
                ...
                if has_systemd_service:
                    ...
                    if restart.returncode == 0:
                        ...
                        if is_linux():
                            ...
                            if linger_ok is not True:
                                ...
                                print("    satan gateway restart")
                            else:
                                print("  Try manually: satan gateway restart")
                elif has_system_service:
                    # System-level service (satan gateway install --system).
                    ...
                    print("  Restart it with: satan gateway run")
        except Exception as e:
            logger.debug("Gateway restart during update failed: %s", e)
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  satan model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``satan -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `satan profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_satan_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → satan -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.satan)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  satan profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     satan -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from satan_constants import get_satan_home
                profile_dir_display = f"~/.satan/profiles/{name}"
        ...
        name = args.profile_name
        from satan_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from satan_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from satan_cli.profiles import rename_profile
        try:
        ...
        from satan_cli.profiles import export_profile
        name = args.profile_name
        ...
        from satan_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from satan_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for satan CLI."""
    parser = argparse.ArgumentParser(
        prog="satan",
        description="Satan Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    satan                        Start interactive chat
    satan chat -q "Hello"        Single query mode
    satan -c                     Resume the most recent session
    satan -c "my project"        Resume a session by name (latest in lineage)
    satan --resume <session_id>  Resume a specific session by ID
    satan setup                  Run setup wizard
    satan logout                 Clear stored authentication
    satan auth add <provider>    Add a pooled credential
    satan auth list              List pooled credentials
    satan auth remove <p> <n>    Remove pooled credential by index
    satan auth reset <provider>  Clear exhaustion status for a provider
    satan model                  Select default model
    satan config                 View configuration
    satan config edit            Edit config in $EDITOR
    satan config set model gpt-4 Set a config value
    satan gateway                Run messaging gateway
    satan -s satan-agent-dev,github-auth
    satan -w                     Start in isolated git worktree
    satan gateway install        Install gateway background service
    satan sessions list          List past sessions
    satan sessions browse        Interactive session picker
    satan sessions rename ID T   Rename/title a session
    satan update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Satan Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: satan-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Satan Agent with an interactive wizard. "
                    "Run a specific section: satan setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Satan CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: satan-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Satan Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Satan Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Satan Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from satan_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from satan_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from satan_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/satan-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from satan_cli.plugins_cmd import plugins_command
    ...
    # Provider selection happens via 'satan memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'satan memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser("setup", help="Initial Honcho setup (redirects to satan memory setup)")
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Satan Honcho",
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'satan memory setup'...\n")
            from satan_cli.memory_setup import memory_command
            memory_command(args)
            return
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'satan tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # satan tools disable <name...> [--platform cli]
    ...
    # satan tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from satan_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Satan as an MCP server",
        description=(
            "Manage MCP server connections and run Satan as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'satan mcp add' to connect to a new server, or\n"
            "'satan mcp serve' to expose Satan conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Satan as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch satan --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            satan_bin = shutil.which("satan")
            if satan_bin:
                os.execvp(satan_bin, ["satan", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "satan_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Satan"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Satan",
    ...
    def cmd_claw(args):
        from satan_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Satan Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Satan Agent",
        description="Remove Satan Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Satan Agent as an ACP (Agent Client Protocol) server",
        description="Start Satan Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Satan Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Satan instances",
    )
    ...
    # e.g. ``satan -c Pokemon Agent Dev`` → ``satan -c 'Pokemon Agent Dev'``
