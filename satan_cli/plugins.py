...
1. **User plugins**   – ``~/.satan/plugins/<name>/``
2. **Project plugins** – ``./.satan/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``satan_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "satan_agent.plugins"

_NS_PARENT = "satan_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.satan/plugins/)
        satan_home = os.environ.get("HERMES_HOME", os.path.expanduser("~/.satan"))
        user_dir = Path(satan_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.satan/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".satan" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``satan_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``satan tools`` TUI so plugin-provided toolsets appear
