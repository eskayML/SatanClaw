"""
Gateway subcommand for satan CLI.

Handles: satan gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from satan_cli.config import get_env_value, get_satan_home, save_env_value, is_managed, managed_error
# display_satan_home is imported lazily at call sites to avoid ImportError
# when satan_constants is cached from a pre-update version during `satan update`.
from satan_cli.setup import (
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "satan_cli.main gateway",
        "satan_cli/main.py gateway",
        "satan gateway",
        "gateway/run.py",
    ]
...
_SERVICE_BASE = "satan-gateway"
SERVICE_DESCRIPTION = "Satan Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HERMES_HOME.

    Returns ``""`` for the default ``~/.satan``, the profile name for
    ``~/.satan/profiles/<name>``, or a short hash for any other custom
    HERMES_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_satan_home().resolve()
    default = (_Path.home() / ".satan").resolve()
    if home == default:
        return ""
    # Detect ~/.satan/profiles/<name> pattern → use the profile name
...
def get_service_name() -> str:
    ...
    Default ``~/.satan`` returns ``satan-gateway`` (backward compatible).
    Profile ``~/.satan/profiles/coder`` returns ``satan-gateway-coder``.
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    satan gateway uninstall")
    print_info("    sudo satan gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Satan can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo satan gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo satan gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo satan gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.satan`` → ``ai.satan.gateway.plist`` (backward compatible).
    Profile ``~/.satan/profiles/coder`` → ``ai.satan.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.satan.gateway-{suffix}" if suffix else "ai.satan.gateway"
...
def get_satan_cli_path() -> str:
    """Get the path to the satan CLI."""
    ...
    return f"{get_python_path()} -m satan_cli.main"
...
def _satan_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HERMES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_satan_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.satan                    → /home/alice/.satan
      /root/.satan/profiles/coder     → /home/alice/.satan/profiles/coder
      /opt/custom-satan               → /opt/custom-satan  (kept as-is)
    """
    current_satan = get_satan_home().resolve()
    current_default = (Path.home() / ".satan").resolve()
    target_default = Path(target_home_dir) / ".satan"

    # Default ~/.satan → remap to target user's default
    if current_satan == current_default:
        return str(target_default)

    # Profile or subdir of ~/.satan → preserve the relative structure
    try:
        relative = current_satan.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.satan) — keep as-is
        return str(current_satan)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        satan_home = _satan_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m satan_cli.main gateway run --replace
...
Environment="HERMES_HOME={satan_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    satan_home = str(get_satan_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Satan install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}satan gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}satan gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}satan gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}satan gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}satan gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.satan.gateway-{suffix}" if suffix else "ai.satan.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Satan install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  satan gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Satan install")
    else:
        print("⚠ Service definition is stale relative to the current Satan install")
        print("  Run: satan gateway start")
    ...
    if deep:
        log_file = get_satan_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Satan will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @satan:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. satan) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Satan delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Satan agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Satan will use (e.g., satan@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_satan_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'satan pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: satan pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'satan gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from satan_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SatanAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: satan gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: satan gateway")
    else:
        print()
        print_info("No platforms configured. Run 'satan gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: satan gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: satan gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  satan gateway install")
                print("  sudo satan gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  satan gateway          # Run in foreground")
                print("  satan gateway install  # Install as user service")
                print("  sudo satan gateway install --system  # Install as boot-time system service")
