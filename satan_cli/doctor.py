"""
Doctor command for satan CLI.

Diagnoses issues with Satan Agent setup.
"""
...
from satan_cli.config import get_project_root, get_satan_home, get_env_path
from satan_constants import display_satan_home

PROJECT_ROOT = get_project_root()
HERMES_HOME = get_satan_home()
_DHH = display_satan_home()  # user-facing display path (e.g. ~/.satan or ~/.satan/profiles/coder)

# Load environment variables from ~/.satan/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.satan/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `satan`.
    ...
    print(color("│                 🩺 Satan Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.satan/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'satan setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'satan setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'satan setup' to create one")
                issues.append("Run 'satan setup' to create .env")
    
    # Check ~/.satan/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    try:
        from satan_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    satan_home = HERMES_HOME
    if satan_home.exists():
    ...
    for subdir_name in expected_subdirs:
        subdir_path = satan_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Satan a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Satan Agent Persona\n\n"
                "<!-- Edit this file to customize how Satan communicates. -->\n\n"
                "You are Satan, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = satan_home / "memories"
    ...
    state_db_path = satan_home / "state.db"
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/satan-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'satan setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: satan skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: satan memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: satan memory setup")
            issues.append("No Honcho API key — run 'satan memory setup'")
        else:
    ...
    # =========================================================================
    try:
        from satan_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "satan -p" in content:
                            _m = _re.search(r"satan -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'satan doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
