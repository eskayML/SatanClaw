"""satan memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as satan tools)
# ---------------------------------------------------------------------------
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.satan/plugins/ and try again.\n")
        return
    ...
    env_path = Path(os.environ.get("HERMES_HOME", os.path.expanduser("~/.satan"))) / ".env"
    env_writes = {}
    ...
    satan_home = str(Path(os.environ.get("HERMES_HOME", os.path.expanduser("~/.satan"))))
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, satan_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.satan/plugins/")
