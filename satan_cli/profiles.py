"""
Profile management for multiple isolated Satan instances.

Each profile is a fully independent HERMES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.satan/profiles/<name>/`` by default.

The "default" profile is ``~/.satan`` itself — backward compatible,
zero migration needed.

Usage::

    satan profile create coder          # fresh profile + bundled skills
    satan profile create coder --clone  # also copy config, .env, SOUL.md
    satan profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    satan -p coder chat                 # or via flag
    satan profile use coder             # set as sticky default
    satan profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.satan) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "satan-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "satan", "default", "test", "tmp", "root", "sudo",
})

# Satan subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.satan/profiles/`` — anchored to the user's home,
    NOT to the current HERMES_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".satan" / "profiles"


def _get_default_satan_home() -> Path:
    """Return the default (pre-profile) HERMES_HOME path."""
    return Path.home() / ".satan"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_satan_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.satan
...
def get_profile_dir(name: str) -> Path:
    ...
    if name == "default":
        return _get_default_satan_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, satan subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a satan subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_satan_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.satan)."
        )
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.satan).\n"
            "To remove everything, use: satan uninstall"
        )
    ...
    # Check for service
    from satan_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from satan_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.satan/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: satan profile create {name}"
        )
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to ``~/.satan``.
    Returns the profile name if HERMES_HOME points into ``~/.satan/profiles/<name>``.
    Returns ``"custom"`` if HERMES_HOME is set to an unrecognized path.
    """
    from satan_constants import get_satan_home
    satan_home = get_satan_home()
    resolved = satan_home.resolve()

    default_resolved = _get_default_satan_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.satan itself — its parent is ~/ and its
        # directory name is ".satan", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: satan profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.satan itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.satan). "
            "Specify a different name: satan profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for satan profile names."""
    return '''# Satan Agent profile completion
# Add to ~/.bashrc: eval "$(satan completion bash)"

_satan_profiles() {
    local profiles_dir="$HOME/.satan/profiles"
...
_satan_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_satan_profiles)" -- "$cur"))
        return
    fi
...
complete -F _satan_completion satan
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for satan profile names."""
    return '''#compdef satan
# Satan Agent profile completion
# Add to ~/.zshrc: eval "$(satan completion zsh)"

_satan() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.satan/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.satan/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any satan modules
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: satan profile create {profile_name}"
        )
