"""Slash command definitions and autocomplete for the Satan CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update Satan Agent to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("HERMES_HOME", os.path.expanduser("~/.satan")),
            "config.yaml",
        )
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /satan handler.

    Maps both canonical names and aliases so /satan bg do stuff works
    the same as /satan background do stuff.
    """
    overrides = _resolve_config_gates()
