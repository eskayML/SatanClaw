"""satan claw — OpenClaw migration commands.

Usage:
    satan claw migrate              # Interactive migration from ~/.openclaw
    satan claw migrate --dry-run    # Preview what would be migrated
    satan claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    satan claw cleanup              # Archive leftover OpenClaw directories
    satan claw cleanup --dry-run    # Preview what would be archived
"""
...
from satan_cli.config import get_satan_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_satan.py script in known locations."""
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: satan claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Satan")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'satan claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Satan migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: satan claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {satan_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: satan claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  satan claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  satan claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  satan config set OPENROUTER_API_KEY sk-or-v1-...")
