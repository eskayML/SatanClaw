"""satan webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.satan/webhook_subscriptions.json and are
...
def _satan_home() -> Path:
    return Path(
        os.getenv("HERMES_HOME", str(Path.home() / ".satan"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _satan_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: satan gateway run
"""
...
def webhook_command(args):
    """Entry point for 'satan webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: satan webhook {subscribe|list|remove|test}")
        print("Run 'satan webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (satan gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: satan webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from satan webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (satan gateway run)")
