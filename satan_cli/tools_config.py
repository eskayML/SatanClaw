"""
Unified tool configuration for Satan Agent.

`satan tools` and `satan setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.satan/config.yaml under
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from satan_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "satan-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "satan-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "satan-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "satan-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "satan-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "satan-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "satan-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "satan-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "satan-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "satan-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "satan-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "satan-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "satan-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "satan-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "satan-webhook"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_satan_home()}/satan-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in satan-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "satan-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "satan-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `satan tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by satan tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (satan-cli, satan-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.satan/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `satan tools` and `satan setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Satan Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from satan_constants import display_satan_home
    print(color(f"  Tool configuration saved to {display_satan_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'satan' or gateway restart.", Colors.DIM))
    print()
