...
from satan_cli.auth import get_nous_auth_status
from satan_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: satan setup tools, satan setup terminal, or satan status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `satan setup` and `satan model`."""
    if not managed_nous_tools_enabled():
        return set()
