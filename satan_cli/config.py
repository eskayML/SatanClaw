"""
Configuration management for Satan Agent.

Config files are stored in ~/.satan/ for easy access:
- ~/.satan/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.satan/.env         - API keys and secrets

This module provides:
- satan config          - Show current configuration
- satan config edit     - Open config in editor
- satan config set      - Set a specific value
- satan config wizard   - Re-run setup wizard
"""
...
from satan_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_satan_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Satan is running in package-manager-managed mode.
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade satan-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "satan update"


def format_managed_message(action: str = "modify this Satan installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Satan installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.satan-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Satan installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade satan-agent"
        )

    return (
        f"Cannot {action}: this Satan installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Satan."
    )
...
# Re-export from satan_constants — canonical definition lives there.
from satan_constants import get_satan_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_satan_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_satan_home() / ".env"
...
def ensure_satan_home():
    """Ensure ~/.satan directory structure exists with secure permissions."""
    home = get_satan_home()
...
DEFAULT_CONFIG = {
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Satan sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.satan/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for satan-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Satan Agent*" header.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Satan auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @satan:example.org)",
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: satan config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.satan/config.yaml."""
    import copy
    ensure_satan_home()
...
#   openai-codex (OAuth — satan login) — OpenAI Codex
#   nous         (OAuth — satan login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.satan/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.satan/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Satan env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.satan/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.satan/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.satan/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Satan Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  satan config edit     # Edit config file", Colors.DIM))
    print(color("  satan config set <key> <value>", Colors.DIM))
    print(color("  satan setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: satan config set <key> <value>")
            print()
            print("Examples:")
            print("  satan config set model anthropic/claude-sonnet-4")
            print("  satan config set terminal.backend docker")
            print("  satan config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'satan config migrate' to add them")
        ...
        print("  satan config           Show current configuration")
        print("  satan config edit      Open config in editor")
        print("  satan config set <key> <value>   Set a config value")
        print("  satan config check     Check for missing/outdated config")
        print("  satan config migrate   Update config with new options")
        print("  satan config path      Show config file path")
        print("  satan config env-path  Show .env file path")
        sys.exit(1)
