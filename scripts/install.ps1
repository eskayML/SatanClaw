# ============================================================================
# SatanClaw Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/NousResearch/satanclaw-agent/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$SatanClawHome = "$env:LOCALAPPDATA\satanclaw",
    [string]$InstallDir = "$env:LOCALAPPDATA\satanclaw\satanclaw-agent"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/satanclaw-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/satanclaw-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ SatanClaw Agent Installer                   │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  An open source AI agent by Samuel Kalu.              │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    $managedNode = "$SatanClawHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$SatanClawHome\node;$env:Path"
        Write-Success "Node.js $version found (SatanClaw-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.satanclaw/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\satanclaw-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$SatanClawHome\node") { Remove-Item -Recurse -Force "$SatanClawHome\node" }
                Move-Item $extractedDir.FullName "$SatanClawHome\node"
                $env:Path = "$SatanClawHome\node;$env:Path"

                $version = & "$SatanClawHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.satanclaw/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/satanclaw-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\satanclaw-agent-$Branch.zip"
                $extractPath = "$env:TEMP\satanclaw-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up satanclaw command..."
    
    if ($NoVenv) {
        $satanclawBin = "$InstallDir"
    } else {
        $satanclawBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so satanclaw is globally available
    # On Windows, the satanclaw.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$satanclawBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$satanclawBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $satanclawBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HERMES_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\satanclaw instead
    # of the Unix default ~/.satanclaw
    $currentSatanClawHome = [Environment]::GetEnvironmentVariable("HERMES_HOME", "User")
    if (-not $currentSatanClawHome -or $currentSatanClawHome -ne $SatanClawHome) {
        [Environment]::SetEnvironmentVariable("HERMES_HOME", $SatanClawHome, "User")
        Write-Success "Set HERMES_HOME=$SatanClawHome"
    }
    $env:HERMES_HOME = $SatanClawHome
    
    # Update current session
    $env:Path = "$satanclawBin;$env:Path"
    
    Write-Success "satanclaw command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.satanclaw directory structure
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SatanClawHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$SatanClawHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.satanclaw/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.satanclaw/.env"
        }
    } else {
        Write-Info "~/.satanclaw/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$SatanClawHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.satanclaw/config.yaml from template"
        }
    } else {
        Write-Info "~/.satanclaw/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$SatanClawHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        @"
# SatanClaw Agent Persona

<!-- 
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how SatanClaw communicates with you.
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.satanclaw/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.satanclaw/"
    
    # Seed bundled skills into ~/.satanclaw/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.satanclaw/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.satanclaw/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SatanClawHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.satanclaw/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    # Run satanclaw setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m satanclaw_cli.main setup
    } else {
        python -m satanclaw_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $satanclawCmd = "$InstallDir\venv\Scripts\satanclaw.exe"
    if (-not (Test-Path $satanclawCmd)) {
        $satanclawCmd = "satanclaw"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SatanClawHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'satanclaw whatsapp' to pair via QR code..."
        Write-Host ""
        $response = Read-Host "Pair WhatsApp now? [Y/n]"
        if ($response -eq "" -or $response -match "^[Yy]") {
            try {
                & $satanclawCmd whatsapp
            } catch {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SatanClawHome\logs\gateway.log"
            Start-Process -FilePath $satanclawCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SatanClawHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: satanclaw gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: satanclaw gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SatanClawHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SatanClawHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SatanClawHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SatanClawHome\satanclaw-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   satanclaw              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   satanclaw setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   satanclaw config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   satanclaw config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   satanclaw gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   satanclaw update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/satanclaw-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
