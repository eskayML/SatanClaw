...
description: "Reference for SatanClaw Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
SatanClaw may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, SatanClaw only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, SatanClaw does not create an empty MCP runtime toolset for that server.
...
Behavior:
- SatanClaw uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.satanclaw/mcp-tokens/<server>.json` and reused across sessions
