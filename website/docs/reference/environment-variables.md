...
description: "Complete reference of all environment variables used by Satan Agent"
---

# Environment Variables Reference

All variables go in `~/.satan/.env`. You can also set them with `satan config set VAR value`.
...
| `HERMES_HOME` | Override Satan config directory (default: `~/.satan`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Satan prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Satan cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.satan/sandboxes/`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@satan:matrix.org`) — required for password login, optional with access token |
...
| `HERMES_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.satan/plugins/` (`true`/`false`, default: `false`) |
...
For task-specific direct endpoints, Satan uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in `~/.satan/config.yaml` under the `provider_routing` section:
...
:::tip
Use `satan config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
