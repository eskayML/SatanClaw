...
description: "Catalog of bundled skills that ship with SatanClaw Agent"
---

# Bundled Skills Catalog

SatanClaw ships with a large built-in skill library copied into `~/.satanclaw/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `satanclaw-agent-spawning` | Spawn additional SatanClaw Agent instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate satanclaw process. | `autonomous-ai-agents/satanclaw-agent` |
...
| `satanclaw-agent-setup` | Help users configure SatanClaw Agent — CLI usage, setup wizard, model/provider selection, tools, skills, voice/STT/TTS, gateway, and troubleshooting. | `dogfood/satanclaw-agent-setup` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native SatanClaw Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `satanclaw-atropos-environments` | Build, test, and debug SatanClaw Agent RL environments for Atropos training. Covers the SatanClawAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/satanclaw-atropos-environments` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the SatanClaw venv. | `productivity/google-workspace` |
...
| `plan` | Plan mode for SatanClaw — inspect context, write a markdown plan into `.satanclaw/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
...
```bash
satanclaw skills install official/<category>/<skill>
```
...
| Skill | Description | Path |
|-------|-------------|------|
| `blender-mcp` | Control Blender directly from SatanClaw via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. | `creative/blender-mcp` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `agentmail` | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. satanclaw-agent@agentmail.to). | `email/agentmail` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `openclaw-migration` | Migrate a user's OpenClaw customization footprint into SatanClaw Agent. Imports SatanClaw-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports what could not be migrated and why. | `migration/openclaw-migration` |

## productivity

| Skill | Description | Path |
|-------|-------------|------|
| `telephony` | Give SatanClaw phone capabilities — provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. | `productivity/telephony` |
