...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to SatanClaw Agent — schemas, handlers, registration, and toolsets"
---
...
If your tool requires an API key, add it to `satanclaw_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `satanclaw_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `satanclaw chat -q "Use the weather tool for London"`
