...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Satan Agent — schemas, handlers, registration, and toolsets"
---
...
If your tool requires an API key, add it to `satan_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `satan_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `satan chat -q "Use the weather tool for London"`
