...
2. **Satan spins up** a fresh agent session with your prompt
...
- **Satan Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.satan/cron/output/` and you can read them anytime.
:::
...
Satan will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Satan Agent
```
...
Satan will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Satan to delegate each topic to a sub-agent:
...
Satan will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
