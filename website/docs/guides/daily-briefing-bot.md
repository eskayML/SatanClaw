...
2. **SatanClaw spins up** a fresh agent session with your prompt
...
- **SatanClaw Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.satanclaw/cron/output/` and you can read them anytime.
:::
...
SatanClaw will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by SatanClaw Agent
```
...
Just tell SatanClaw what you want:
...
SatanClaw will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell SatanClaw to delegate each topic to a sub-agent:
...
SatanClaw will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
