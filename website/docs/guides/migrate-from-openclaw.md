...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Satan Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`satan claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Satan. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | Satan destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.satan/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.satan/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.satan/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.satan/memories/MEMORY.md` | All daily files merged into main memory. |

All workspace files also check `workspace.default/` as a fallback path.

### Skills (4 sources)

| Source | OpenClaw location | Satan destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.satan/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.satan/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.satan/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.satan/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Satan skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Satan destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.satan/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Satan config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Satan config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Satan field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.satan/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Satan `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Satan path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Satan equivalent)

These are saved to `~/.satan/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Satan |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Satan has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `satan cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](../user-guide/features/hooks.md) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `satan webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `satan honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `satan skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Satan profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env`. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — those values must be added to Satan manually after migration.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.satan/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Verify API keys** — run `satan status` to check provider authentication.

4. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart satan-gateway`

5. **Check session policies** — verify `satan config get session_reset` matches your expectations.

6. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `satan whatsapp` to pair.
...
Imported skills land in `~/.satan/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `satan config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
