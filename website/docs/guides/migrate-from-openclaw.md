...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to SatanClaw Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`satanclaw claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into SatanClaw. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | SatanClaw destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.satanclaw/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.satanclaw/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.satanclaw/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.satanclaw/memories/MEMORY.md` | All daily files merged into main memory. |

All workspace files also check `workspace.default/` as a fallback path.

### Skills (4 sources)

| Source | OpenClaw location | SatanClaw destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.satanclaw/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.satanclaw/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.satanclaw/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.satanclaw/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing SatanClaw skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | SatanClaw destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.satanclaw/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | SatanClaw config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | SatanClaw config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | SatanClaw field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.satanclaw/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | SatanClaw `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | SatanClaw path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct SatanClaw equivalent)

These are saved to `~/.satanclaw/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in SatanClaw |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | SatanClaw has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `satanclaw cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](../user-guide/features/hooks.md) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `satanclaw webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `satanclaw honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `satanclaw skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use SatanClaw profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env`. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — those values must be added to SatanClaw manually after migration.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.satanclaw/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Verify API keys** — run `satanclaw status` to check provider authentication.

4. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart satanclaw-gateway`

5. **Check session policies** — verify `satanclaw config get session_reset` matches your expectations.

6. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `satanclaw whatsapp` to pair.
...
Imported skills land in `~/.satanclaw/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `satanclaw config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
