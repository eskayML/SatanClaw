...
title: "Git Worktrees"
description: "Run multiple SatanClaw agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

SatanClaw Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with SatanClaw so each session has a clean, isolated working directory.

## Why Use Worktrees with SatanClaw?

SatanClaw treats the **current working directory** as the project root:

- CLI: the directory where you run `satanclaw` or `satanclaw chat`
...
git worktree add ../repo-feature feature/satanclaw-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/satanclaw-experiment` checked out in that directory

Now you can `cd` into the new worktree and run SatanClaw there:

```bash
cd ../repo-feature

# Start SatanClaw in the worktree
satanclaw
```
...
git worktree add ../repo-experiment-a feature/satanclaw-a
git worktree add ../repo-experiment-b feature/satanclaw-b
```
...
Each SatanClaw process:

- Works on its own branch (`feature/satanclaw-a` vs `feature/satanclaw-b`).
...
- SatanClaw checkpoint data under `~/.satanclaw/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per SatanClaw experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/satanclaw-checkpoints-docs`, `feature/satanclaw-refactor-tests`.
- **Commit frequently**
...
- **Avoid running SatanClaw from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `satanclaw -w` (Automatic Worktree Mode)

SatanClaw has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `satanclaw/satanclaw-<hash>`).
...
```bash
satanclaw -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `satanclaw -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each SatanClaw session its own clean checkout.
