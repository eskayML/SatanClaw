...
title: "Git Worktrees"
description: "Run multiple Satan agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Satan Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Satan so each session has a clean, isolated working directory.

## Why Use Worktrees with Satan?

Satan treats the **current working directory** as the project root:

- CLI: the directory where you run `satan` or `satan chat`
...
git worktree add ../repo-feature feature/satan-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/satan-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Satan there:

```bash
cd ../repo-feature

# Start Satan in the worktree
satan
```
...
git worktree add ../repo-experiment-a feature/satan-a
git worktree add ../repo-experiment-b feature/satan-b
```
...
- Works on its own branch (`feature/satan-a` vs `feature/satan-b`).
...
- Satan checkpoint data under `~/.satan/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Satan experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/satan-checkpoints-docs`, `feature/satan-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Satan from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `satan -w` (Automatic Worktree Mode)

Satan has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `satan/satan-<hash>`).
...
```bash
satan -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `satan -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Satan session its own clean checkout.
