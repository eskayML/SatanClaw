...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with SatanClaw Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

SatanClaw Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with SatanClaw](/docs/guides/use-voice-mode-with-satanclaw).

## Prerequisites

Before using voice features, make sure you have:

1. **SatanClaw Agent installed** — `pip install satanclaw-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `satanclaw model` or set your preferred provider credentials in `~/.satanclaw/.env`
3. **A working base setup** — run `satanclaw` to verify the agent responds to text before enabling voice

:::tip
The `~/.satanclaw/` directory and default `config.yaml` are created automatically the first time you run `satanclaw`. You only need to create `~/.satanclaw/.env` manually for API keys.
:::
...
```bash
satanclaw                # Start the interactive CLI
```
...
1. Start the CLI with `satanclaw` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.satanclaw/config.yaml` (default: `ctrl+b`).
:::
...
```bash
satanclaw gateway        # Start the gateway (connects to configured platforms)
satanclaw gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@satanclawbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.satanclaw/.env`:
```bash
...
```bash
satanclaw gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.satanclaw/.env`
...
- Review gateway logs: `tail -f ~/.satanclaw/logs/gateway.log`
