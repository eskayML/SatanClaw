...
Satan Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Satan checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.satan/config.yaml`:
...
1. **CLI flag**: Start a session with `satan --yolo` or `satan chat --yolo`
...
Configure the timeout in `~/.satan/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.satan/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.satan/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` satan/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.satan/config.yaml`:
...
:::tip
Use `satan config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Satan controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.satan/.env`:
...
For more flexible authorization, Satan includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `satan pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.satan/config.yaml`:
...
satan pairing approve telegram ABC12DEF

# Revoke a user's access
satan pairing revoke telegram 123456789

# Clear all pending codes
satan pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.satan/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Satan applies strict security hardening to every container.
...
Container resources are configurable in `~/.satan/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.satan/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` in `~/.satan/`. Skills declare these in frontmatter:
...
When loaded, Satan checks if these files exist in `~/.satan/` and registers them for mounting:
...
Paths are relative to `~/.satan/`. Files are mounted to `/root/.satan/` inside the container.
...
| **terminal** (local) | Blocks explicit Satan infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Satan infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Satan integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.satan/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.satan/logs/` for unauthorized access attempts
10. **Keep updated** — run `satan update` regularly for security patches
