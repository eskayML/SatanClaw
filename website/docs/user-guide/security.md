...
SatanClaw Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, SatanClaw checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.satanclaw/config.yaml`:
...
1. **CLI flag**: Start a session with `satanclaw --yolo` or `satanclaw chat --yolo`
...
Configure the timeout in `~/.satanclaw/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.satanclaw/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.satanclaw/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` satanclaw/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.satanclaw/config.yaml`:
...
:::tip
Use `satanclaw config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, SatanClaw controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.satanclaw/.env`:
...
For more flexible authorization, SatanClaw includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `satanclaw pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.satanclaw/config.yaml`:
...
satanclaw pairing approve telegram ABC12DEF

# Revoke a user's access
satanclaw pairing revoke telegram 123456789

# Clear all pending codes
satanclaw pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.satanclaw/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, SatanClaw applies strict security hardening to every container.
...
Container resources are configurable in `~/.satanclaw/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.satanclaw/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` in `~/.satanclaw/`. Skills declare these in frontmatter:
...
When loaded, SatanClaw checks if these files exist in `~/.satanclaw/` and registers them for mounting:
...
Paths are relative to `~/.satanclaw/`. Files are mounted to `/root/.satanclaw/` inside the container.
...
| **terminal** (local) | Blocks explicit SatanClaw infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- SatanClaw infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
SatanClaw integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.satanclaw/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.satanclaw/logs/` for unauthorized access attempts
10. **Keep updated** — run `satanclaw update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.satanclaw/.env
...
terminal:
  ...
  ssh_user: "satanclaw"
  ssh_key: "~/.ssh/satanclaw_agent_key"
```
