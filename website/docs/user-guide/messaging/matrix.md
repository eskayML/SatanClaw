...
| **Rooms** | SatanClaw responds to all messages in rooms it has joined. Room invites are auto-accepted. |
| **Threads** | SatanClaw supports Matrix threads (MSC3440). If you reply in a thread, SatanClaw keeps the thread context isolated from the main room timeline. |
| **Shared rooms with multiple users** | By default, SatanClaw isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `satanclaw` — the full user ID will be `@satanclaw:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `satanclaw-bot`).

### Option C: Use Your Own Account

You can also run SatanClaw as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

SatanClaw needs an access token to authenticate with the homeserver. You have two options:
...
curl -X POST https://your-server/_matrix/client/v3/login \
  ...
  -d '{
    ...
    "user": "@satanclaw:your-server.org",
...
Instead of providing an access token, you can give SatanClaw the bot's user ID and password. SatanClaw will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@satanclaw:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

SatanClaw Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure SatanClaw Agent
...
# MATRIX_USER_ID=@satanclaw:matrix.example.org
...
SatanClaw supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with satanclaw extras
pip install 'satanclaw-agent[matrix]'
```
...
Add to your `~/.satanclaw/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, SatanClaw:

- Stores encryption keys in `~/.satanclaw/matrix/store/`
...
:::warning
If you delete the `~/.satanclaw/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
Add this to your `~/.satanclaw/.env`:
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the SatanClaw logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The SatanClaw gateway isn't running, or it failed to connect.

**Fix**: Check that `satanclaw gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.satanclaw/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: SatanClaw can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
