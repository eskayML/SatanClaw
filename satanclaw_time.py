"""
Timezone-aware clock for SatanClaw.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.satanclaw/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — SatanClaw never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — SatanClaw requires 3.9+)
