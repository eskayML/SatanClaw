"""
SatanClawSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser satanclaw
    run-api
    python environments/satanclaw_swe_env.py serve \\
    ...
    python environments/satanclaw_swe_env.py serve \\
        ...
        --env.tool_call_parser satanclaw \\
        --env.terminal_backend modal
"""
...
class SatanClawSweEnv(SatanClawAgentBaseEnv):
    ...
    name = "satanclaw-swe"
    env_config_cls = SatanClawSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[SatanClawSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        """
        env_config = SatanClawSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepSatanClaw-3-Llama-3-3B-Preview",
            tool_call_parser="satanclaw",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="satanclaw-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepSatanClaw-3-Llama-3-3B-Preview",
