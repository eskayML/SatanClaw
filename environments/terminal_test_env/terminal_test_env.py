...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Satan Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Satan Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Satan Agent",
    },
    {
...
class TerminalTestEnv(SatanAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/satan-agent/.env.
        """
