...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from SatanClaw Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from SatanClaw Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from SatanClaw Agent",
    },
    {
...
class TerminalTestEnv(SatanClawAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/satanclaw-agent/.env.
        """
