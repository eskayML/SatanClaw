...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      satanclaw-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "satanclaw-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${satanclawVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "satanclaw-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${satanclaw-agent}/bin/satanclaw || (echo "FAIL: satanclaw binary missing"; exit 1)
          test -x ${satanclaw-agent}/bin/satanclaw-agent || (echo "FAIL: satanclaw-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${satanclaw-agent}/bin/satanclaw version 2>&1 | grep -qi "satanclaw" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "satanclaw-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in satanclaw satanclaw-agent satanclaw-acp; do
            test -x ${satanclaw-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "satanclaw-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking satanclaw --help ==="
          ${satanclaw-agent}/bin/satanclaw --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${satanclaw-agent}/bin/satanclaw --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "satanclaw-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${satanclaw-agent}/share/satanclaw-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${satanclaw-agent}/share/satanclaw-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${satanclaw-agent}/bin/satanclaw || \
        ...
        managed-guard = pkgs.runCommand "satanclaw-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${satanclaw-agent}/bin/satanclaw config set model foo
          check_blocked "config edit" ${satanclaw-agent}/bin/satanclaw config edit
        ...
        in pkgs.runCommand "satanclaw-config-roundtrip" {
          ...
          merge_and_load() {
            local satanclaw_home="$1"
            export HERMES_HOME="$satanclaw_home"
            ${configMergeScript} ${nixSettings} "$satanclaw_home/config.yaml"
            ${satanclawVenv}/bin/python3 -c '
import json, sys
