# nix/nixosModules.nix — NixOS module for satanclaw-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: satanclaw runs from /nix/store bind-mounted read-only into a
...
# and read by satanclaw at startup — no container recreation needed for env changes.
#
# Tool resolution: the satanclaw wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.satanclaw-agent;
    satanclaw-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-satanclaw-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "satanclaw-config-attrs";
      description = "SatanClaw YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "satanclaw-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "satanclaw-documents" { } (
      ''
        mkdir -p $out
    ...
    containerName = "satanclaw-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/satanclaw";
    ...
    # satanclaw user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "satanclaw-container-entrypoint" ''
      set -eu
      ...
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        TARGET_USER="satanclaw"
        TARGET_HOME="/home/satanclaw"
      ...
      if [ ! -f /var/lib/satanclaw-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/satanclaw-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/satanclaw ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/satanclaw
        chmod 0440 /etc/sudoers.d/satanclaw
      fi
    ...
    # Default: /var/lib/satanclaw/workspace → /data/workspace.
  ...
  in {
    options.services.satanclaw-agent = with lib; {
      enable = mkEnableOption "SatanClaw Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = satanclaw-agent;
        description = "The satanclaw-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/satanclaw";
        description = "State directory. Contains .satanclaw/ subdir (HERMES_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative SatanClaw config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          SatanClaw reads this file on every startup via load_satanclaw_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `satanclaw gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        type = types.bool;
        default = false;
        description = "Add satanclaw CLI to environment.systemPackages.";
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.satanclaw-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.satanclaw        0750 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."satanclaw-agent-setup" = lib.stringAfter [ "users" ] ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.satanclaw
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.satanclaw ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 0750 ${cfg.stateDir} ${cfg.stateDir}/.satanclaw ${cfg.stateDir}/home ${cfg.workingDirectory}

          # Merge Nix settings into existing config.yaml.
          # Preserves user-added keys (skills, streaming, etc.); Nix keys win.
          # If configFile is user-provided (not generated), overwrite instead of merge.
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.satanclaw/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.satanclaw/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.satanclaw/config.yaml
            chmod 0640 ${cfg.stateDir}/.satanclaw/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.satanclaw/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.satanclaw/.managed
          chmod 0644 ${cfg.stateDir}/.satanclaw/.managed

          # Seed auth file if provided
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.satanclaw/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.satanclaw/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.satanclaw/auth.json
              fi
            ''}
          ''}
          ...
          # SatanClaw reads $HERMES_HOME/.env at startup via load_satanclaw_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.satanclaw/.env"
...
HERMES_NIX_ENV_EOF
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.satanclaw-agent = {
          description = "SatanClaw Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            HERMES_HOME = "${cfg.stateDir}/.satanclaw";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $HERMES_HOME/.env by the activation script. load_satanclaw_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/satanclaw"
              "gateway"
            ] ++ cfg.extraArgs);
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.satanclaw-agent = {
          description = "SatanClaw Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env HERMES_HOME=${containerDataDir}/.satanclaw \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/satanclaw gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
