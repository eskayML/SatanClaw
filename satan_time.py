"""
Timezone-aware clock for Satan.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.satan/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Satan never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Satan requires 3.9+)
