"""Slash command definitions and autocomplete for the SatanClaw CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update SatanClaw Agent to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("HERMES_HOME", os.path.expanduser("~/.satanclaw")),
            "config.yaml",
        )
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /satanclaw handler.

    Maps both canonical names and aliases so /satanclaw bg do stuff works
    the same as /satanclaw background do stuff.
    """
    overrides = _resolve_config_gates()
