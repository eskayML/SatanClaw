#!/usr/bin/env python3
"""
SatanClaw CLI - Main entry point.

Usage:
    satanclaw                     # Interactive chat (default)
    satanclaw chat                # Interactive chat
    satanclaw gateway             # Run gateway in foreground
    satanclaw gateway start       # Start gateway as service
    satanclaw gateway stop        # Stop gateway service
    satanclaw gateway status      # Show gateway status
    satanclaw gateway install     # Install gateway service
    satanclaw gateway uninstall   # Uninstall gateway service
    satanclaw setup               # Interactive setup wizard
    satanclaw logout              # Clear stored authentication
    satanclaw status              # Show status of all components
    satanclaw cron                # Manage cron jobs
    satanclaw cron list           # List cron jobs
    satanclaw cron status         # Check if cron scheduler is running
    satanclaw doctor              # Check configuration and dependencies
    satanclaw honcho setup                    # Configure Honcho AI memory integration
    satanclaw honcho status                   # Show Honcho config and connection status
    satanclaw honcho sessions                 # List directory → session name mappings
    satanclaw honcho map <name>               # Map current directory to a session name
    satanclaw honcho peer                     # Show peer names and dialectic settings
    satanclaw honcho peer --user NAME         # Set user peer name
    satanclaw honcho peer --ai NAME           # Set AI peer name
    satanclaw honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    satanclaw honcho mode                     # Show current memory mode
    satanclaw honcho mode [hybrid|honcho|local]  # Set memory mode
    satanclaw honcho tokens                   # Show token budget settings
    satanclaw honcho tokens --context N       # Set session.context() token cap
    satanclaw honcho tokens --dialectic N     # Set dialectic result char cap
    satanclaw honcho identity                 # Show AI peer identity representation
    satanclaw honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    satanclaw honcho migrate                  # Step-by-step migration guide: OpenClaw native → SatanClaw + Honcho
    satanclaw version             Show version
    satanclaw update              Update to latest version
    satanclaw uninstall           Uninstall SatanClaw Agent
    satanclaw acp                 Run as an ACP server for editor integration
    satanclaw sessions browse     Interactive session picker with search

    satanclaw claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (satanclaw tools, satanclaw setup, satanclaw model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'satanclaw {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any satanclaw module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.satanclaw/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check ~/.satanclaw/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".satanclaw" / "active_profile"
    ...
    if profile_name is not None:
        try:
            from satanclaw_cli.profiles import resolve_profile_env
            satanclaw_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent satanclaw from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["HERMES_HOME"] = satanclaw_home
...
load_satanclaw_dotenv(project_env=PROJECT_ROOT / '.env')
...
from satanclaw_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from satanclaw_cli.config import get_env_path, get_satanclaw_home, load_config
    from satanclaw_cli.auth import get_auth_status

    # Determine whether SatanClaw itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_satanclaw_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_satanclaw_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if SatanClaw has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants SatanClaw to use their tokens.
    if _has_satanclaw_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'satanclaw sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like SatanClaw isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  satanclaw setup")
        print()

        from satanclaw_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'satanclaw setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from satanclaw_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from satanclaw_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from satanclaw_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with SatanClaw?")
        print()
    ...
    session_dir = get_satanclaw_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: satanclaw gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  satanclaw gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ SatanClaw Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  satanclaw gateway")
        ...
        print()
        print("  Or install as a service: satanclaw gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'satanclaw whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``satanclaw model``) and the setup wizard
    ...
    from satanclaw_cli.config import load_config, get_env_value
    ...
    providers = [
        ...
        ("nous", "Nous Portal (Samuel Kalu subscription)"),
        ("openai-codex", "OpenAI Codex"),
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from satanclaw_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from satanclaw_cli.config import get_env_value, save_env_value
    ...
    from satanclaw_cli.models import model_ids
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from satanclaw_cli.config import get_env_value, save_config, save_env_value
    from satanclaw_cli.nous_subscription import (
    ...
    from satanclaw_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from satanclaw_cli.auth import _save_model_choice, deactivate_provider
    from satanclaw_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from satanclaw_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"SatanClaw will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `satanclaw model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from satanclaw_cli.auth import _save_model_choice, deactivate_provider
    from satanclaw_cli.config import save_env_value, load_config, save_config
    from satanclaw_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from satanclaw_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from satanclaw_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates SatanClaw turns to `copilot --acp`.")
    print("  SatanClaw currently starts its own ACP subprocess for each request.")
    print("  SatanClaw uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from satanclaw_cli.config import get_env_value, save_env_value, load_config, save_config
    from satanclaw_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    SatanClaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               satanclaw model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'satanclaw model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate SatanClaw CLI with a provider."""
    from satanclaw_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from satanclaw_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from satanclaw_cli.auth_commands import auth_command
    auth_command(args)
...
def cmd_cron(args):
    """Cron job management."""
    from satanclaw_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from satanclaw_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from satanclaw_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from satanclaw_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"SatanClaw Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from satanclaw_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall SatanClaw Agent."""
    _require_tty("uninstall")
    from satanclaw_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _update_via_zip(args):
    """Update SatanClaw Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/satanclaw-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="satanclaw-update-")
        zip_path = os.path.join(tmp_dir, f"satanclaw-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to satanclaw-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"satanclaw-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("satanclaw-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if SatanClaw behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so SatanClaw can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but SatanClaw couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but SatanClaw couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if SatanClaw behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `satanclaw update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/satanclaw-agent.git",
    "git@github.com:NousResearch/satanclaw-agent.git",
    "https://github.com/NousResearch/satanclaw-agent",
    "git@github.com:NousResearch/satanclaw-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/satanclaw-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_satanclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from satanclaw_constants import get_satanclaw_home
        (get_satanclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official SatanClaw repository.")
        print("  This means you may miss updates from NousResearch/satanclaw-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/satanclaw-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/satanclaw-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``satanclaw update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".satanclaw"
    homes.append(default_home)
    # Named profiles under ~/.satanclaw/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "satanclaw-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update SatanClaw Agent to the latest version."""
    import shutil
    from satanclaw_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update SatanClaw Agent")
        return
    
    print("⚕ Updating SatanClaw Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/satanclaw-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_satanclaw_home added to satanclaw_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload satanclaw_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_satanclaw_home() added since the last release.
        try:
            import importlib
            import satanclaw_constants as _hc
        ...
        try:
            from satanclaw_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                ...
                print("    Run 'satanclaw config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'satanclaw config migrate' later to configure.")
        else:
        ...
        try:
            ...
            from satanclaw_cli.gateway import (
            ...
            # Also check for a system-level service (satanclaw gateway install --system).
            ...
            if is_macos():
                try:
                    from satanclaw_cli.gateway import get_launchd_label
            ...
            if existing_pid or has_systemd_service or has_system_service or has_launchd_service:
                ...
                if has_systemd_service:
                    ...
                    if restart.returncode == 0:
                        ...
                        if is_linux():
                            ...
                            if linger_ok is not True:
                                ...
                                print("    satanclaw gateway restart")
                            else:
                                print("  Try manually: satanclaw gateway restart")
                elif has_system_service:
                    # System-level service (satanclaw gateway install --system).
                    ...
                    print("  Restart it with: satanclaw gateway run")
        except Exception as e:
            logger.debug("Gateway restart during update failed: %s", e)
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  satanclaw model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``satanclaw -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `satanclaw profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_satanclaw_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → satanclaw -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.satanclaw)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  satanclaw profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     satanclaw -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from satanclaw_constants import get_satanclaw_home
                profile_dir_display = f"~/.satanclaw/profiles/{name}"
        ...
        name = args.profile_name
        from satanclaw_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from satanclaw_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec satanclaw -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from satanclaw_cli.profiles import rename_profile
        try:
        ...
        from satanclaw_cli.profiles import export_profile
        name = args.profile_name
        ...
        from satanclaw_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from satanclaw_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for satanclaw CLI."""
    parser = argparse.ArgumentParser(
        prog="satanclaw",
        description="SatanClaw Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    satanclaw                        Start interactive chat
    satanclaw chat -q "Hello"        Single query mode
    satanclaw -c                     Resume the most recent session
    satanclaw -c "my project"        Resume a session by name (latest in lineage)
    satanclaw --resume <session_id>  Resume a specific session by ID
    satanclaw setup                  Run setup wizard
    satanclaw logout                 Clear stored authentication
    satanclaw auth add <provider>    Add a pooled credential
    satanclaw auth list              List pooled credentials
    satanclaw auth remove <p> <n>    Remove pooled credential by index
    satanclaw auth reset <provider>  Clear exhaustion status for a provider
    satanclaw model                  Select default model
    satanclaw config                 View configuration
    satanclaw config edit            Edit config in $EDITOR
    satanclaw config set model gpt-4 Set a config value
    satanclaw gateway                Run messaging gateway
    satanclaw -s satanclaw-agent-dev,github-auth
    satanclaw -w                     Start in isolated git worktree
    satanclaw gateway install        Install gateway background service
    satanclaw sessions list          List past sessions
    satanclaw sessions browse        Interactive session picker
    satanclaw sessions rename ID T   Rename/title a session
    satanclaw update                 Update to latest version

For more help on a command:
    satanclaw <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with SatanClaw Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: satanclaw-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure SatanClaw Agent with an interactive wizard. "
                    "Run a specific section: satanclaw setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for SatanClaw CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: satanclaw-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of SatanClaw Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with SatanClaw Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage SatanClaw Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from satanclaw_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from satanclaw_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from satanclaw_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/satanclaw-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from satanclaw_cli.plugins_cmd import plugins_command
    ...
    # Provider selection happens via 'satanclaw memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'satanclaw memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser("setup", help="Initial Honcho setup (redirects to satanclaw memory setup)")
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to SatanClaw Honcho",
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'satanclaw memory setup'...\n")
            from satanclaw_cli.memory_setup import memory_command
            memory_command(args)
            return
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'satanclaw tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # satanclaw tools list [--platform cli]
    ...
    # satanclaw tools disable <name...> [--platform cli]
    ...
    # satanclaw tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from satanclaw_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run SatanClaw as an MCP server",
        description=(
            "Manage MCP server connections and run SatanClaw as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'satanclaw mcp add' to connect to a new server, or\n"
            "'satanclaw mcp serve' to expose SatanClaw conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run SatanClaw as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch satanclaw --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            satanclaw_bin = shutil.which("satanclaw")
            if satanclaw_bin:
                os.execvp(satanclaw_bin, ["satanclaw", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "satanclaw_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to SatanClaw"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to SatanClaw",
    ...
    def cmd_claw(args):
        from satanclaw_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update SatanClaw Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall SatanClaw Agent",
        description="Remove SatanClaw Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run SatanClaw Agent as an ACP (Agent Client Protocol) server",
        description="Start SatanClaw Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch SatanClaw Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated SatanClaw instances",
    )
    ...
    # e.g. ``satanclaw -c Pokemon Agent Dev`` → ``satanclaw -c 'Pokemon Agent Dev'``
