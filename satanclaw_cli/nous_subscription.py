...
from satanclaw_cli.auth import get_nous_auth_status
from satanclaw_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: satanclaw setup tools, satanclaw setup terminal, or satanclaw status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `satanclaw setup` and `satanclaw model`."""
    if not managed_nous_tools_enabled():
        return set()
