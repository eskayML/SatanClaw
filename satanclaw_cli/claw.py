"""satanclaw claw — OpenClaw migration commands.

Usage:
    satanclaw claw migrate              # Interactive migration from ~/.openclaw
    satanclaw claw migrate --dry-run    # Preview what would be migrated
    satanclaw claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    satanclaw claw cleanup              # Archive leftover OpenClaw directories
    satanclaw claw cleanup --dry-run    # Preview what would be archived
"""
...
from satanclaw_cli.config import get_satanclaw_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_satanclaw.py script in known locations."""
...
def claw_command(args):
    """Route satanclaw claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: satanclaw claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to SatanClaw")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'satanclaw claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → SatanClaw migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: satanclaw claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {satanclaw_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: satanclaw claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  satanclaw claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  satanclaw claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  satanclaw config set OPENROUTER_API_KEY sk-or-v1-...")
