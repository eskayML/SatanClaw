"""
Gateway subcommand for satanclaw CLI.

Handles: satanclaw gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from satanclaw_cli.config import get_env_value, get_satanclaw_home, save_env_value, is_managed, managed_error
# display_satanclaw_home is imported lazily at call sites to avoid ImportError
# when satanclaw_constants is cached from a pre-update version during `satanclaw update`.
from satanclaw_cli.setup import (
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "satanclaw_cli.main gateway",
        "satanclaw_cli/main.py gateway",
        "satanclaw gateway",
        "gateway/run.py",
    ]
...
_SERVICE_BASE = "satanclaw-gateway"
SERVICE_DESCRIPTION = "SatanClaw Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HERMES_HOME.

    Returns ``""`` for the default ``~/.satanclaw``, the profile name for
    ``~/.satanclaw/profiles/<name>``, or a short hash for any other custom
    HERMES_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_satanclaw_home().resolve()
    default = (_Path.home() / ".satanclaw").resolve()
    if home == default:
        return ""
    # Detect ~/.satanclaw/profiles/<name> pattern → use the profile name
...
def get_service_name() -> str:
    ...
    Default ``~/.satanclaw`` returns ``satanclaw-gateway`` (backward compatible).
    Profile ``~/.satanclaw/profiles/coder`` returns ``satanclaw-gateway-coder``.
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    satanclaw gateway uninstall")
    print_info("    sudo satanclaw gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so SatanClaw can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo satanclaw gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo satanclaw gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo satanclaw gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.satanclaw`` → ``ai.satanclaw.gateway.plist`` (backward compatible).
    Profile ``~/.satanclaw/profiles/coder`` → ``ai.satanclaw.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.satanclaw.gateway-{suffix}" if suffix else "ai.satanclaw.gateway"
...
def get_satanclaw_cli_path() -> str:
    """Get the path to the satanclaw CLI."""
    ...
    return f"{get_python_path()} -m satanclaw_cli.main"
...
def _satanclaw_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HERMES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_satanclaw_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.satanclaw                    → /home/alice/.satanclaw
      /root/.satanclaw/profiles/coder     → /home/alice/.satanclaw/profiles/coder
      /opt/custom-satanclaw               → /opt/custom-satanclaw  (kept as-is)
    """
    current_satanclaw = get_satanclaw_home().resolve()
    current_default = (Path.home() / ".satanclaw").resolve()
    target_default = Path(target_home_dir) / ".satanclaw"

    # Default ~/.satanclaw → remap to target user's default
    if current_satanclaw == current_default:
        return str(target_default)

    # Profile or subdir of ~/.satanclaw → preserve the relative structure
    try:
        relative = current_satanclaw.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.satanclaw) — keep as-is
        return str(current_satanclaw)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        satanclaw_home = _satanclaw_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m satanclaw_cli.main gateway run --replace
...
Environment="HERMES_HOME={satanclaw_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    satanclaw_home = str(get_satanclaw_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current SatanClaw install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}satanclaw gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}satanclaw gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}satanclaw gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}satanclaw gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}satanclaw gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.satanclaw.gateway-{suffix}" if suffix else "ai.satanclaw.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    satanclaw_home = str(get_satanclaw_home().resolve())
    log_dir = get_satanclaw_home() / "logs"
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>satanclaw_cli.main</string>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current SatanClaw install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  satanclaw gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current SatanClaw install")
    else:
        print("⚠ Service definition is stale relative to the current SatanClaw install")
        print("  Run: satanclaw gateway start")
    ...
    if deep:
        log_file = get_satanclaw_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ SatanClaw Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and SatanClaw will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @satanclaw:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. satanclaw) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where SatanClaw delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your SatanClaw agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address SatanClaw will use (e.g., satanclaw@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_satanclaw_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'satanclaw pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: satanclaw pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'satanclaw gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from satanclaw_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SatanClawAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: satanclaw gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: satanclaw gateway")
    else:
        print()
        print_info("No platforms configured. Run 'satanclaw gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: satanclaw gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: satanclaw gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  satanclaw gateway install")
                print("  sudo satanclaw gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  satanclaw gateway          # Run in foreground")
                print("  satanclaw gateway install  # Install as user service")
                print("  sudo satanclaw gateway install --system  # Install as boot-time system service")
