"""
Configuration management for SatanClaw Agent.

Config files are stored in ~/.satanclaw/ for easy access:
- ~/.satanclaw/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.satanclaw/.env         - API keys and secrets

This module provides:
- satanclaw config          - Show current configuration
- satanclaw config edit     - Open config in editor
- satanclaw config set      - Set a specific value
- satanclaw config wizard   - Re-run setup wizard
"""
...
from satanclaw_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_satanclaw_home() / ".managed"
...
def is_managed() -> bool:
    """Check if SatanClaw is running in package-manager-managed mode.
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade satanclaw-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "satanclaw update"


def format_managed_message(action: str = "modify this SatanClaw installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this SatanClaw installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.satanclaw-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this SatanClaw installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade satanclaw-agent"
        )

    return (
        f"Cannot {action}: this SatanClaw installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall SatanClaw."
    )
...
# Re-export from satanclaw_constants — canonical definition lives there.
from satanclaw_constants import get_satanclaw_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_satanclaw_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_satanclaw_home() / ".env"
...
def ensure_satanclaw_home():
    """Ensure ~/.satanclaw directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    ...
    "browser": {
        ...
        "camofox": {
            # When true, SatanClaw sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.satanclaw/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for satanclaw-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *SatanClaw Agent*" header.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. eskayML.com -> firecrawl-gateway.eskayML.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the SatanClaw auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @satanclaw:example.org)",
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: satanclaw config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.satanclaw/config.yaml."""
    import copy
...
#   openai-codex (OAuth — satanclaw login) — OpenAI Codex
#   nous         (OAuth — satanclaw login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.satanclaw/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.satanclaw/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real SatanClaw env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.satanclaw/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.satanclaw/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.satanclaw/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ SatanClaw Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  satanclaw config edit     # Edit config file", Colors.DIM))
    print(color("  satanclaw config set <key> <value>", Colors.DIM))
    print(color("  satanclaw setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: satanclaw config set <key> <value>")
            print()
            print("Examples:")
            print("  satanclaw config set model anthropic/claude-sonnet-4")
            print("  satanclaw config set terminal.backend docker")
            print("  satanclaw config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'satanclaw config migrate' to add them")
        ...
        print("  satanclaw config           Show current configuration")
        print("  satanclaw config edit      Open config in editor")
        print("  satanclaw config set <key> <value>   Set a config value")
        print("  satanclaw config check     Check for missing/outdated config")
        print("  satanclaw config migrate   Update config with new options")
        print("  satanclaw config path      Show config file path")
        print("  satanclaw config env-path  Show .env file path")
        sys.exit(1)
