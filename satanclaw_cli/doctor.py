"""
Doctor command for satanclaw CLI.

Diagnoses issues with SatanClaw Agent setup.
"""
...
from satanclaw_cli.config import get_project_root, get_satanclaw_home, get_env_path
from satanclaw_constants import display_satanclaw_home

PROJECT_ROOT = get_project_root()
HERMES_HOME = get_satanclaw_home()
_DHH = display_satanclaw_home()  # user-facing display path (e.g. ~/.satanclaw or ~/.satanclaw/profiles/coder)

# Load environment variables from ~/.satanclaw/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.satanclaw/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `satanclaw`.
    ...
    print(color("│                 🩺 SatanClaw Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.satanclaw/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'satanclaw setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'satanclaw setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'satanclaw setup' to create one")
                issues.append("Run 'satanclaw setup' to create .env")
    
    # Check ~/.satanclaw/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    try:
        from satanclaw_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    satanclaw_home = HERMES_HOME
    if satanclaw_home.exists():
    ...
    for subdir_name in expected_subdirs:
        subdir_path = satanclaw_home / subdir_name
    ...
    soul_path = satanclaw_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give SatanClaw a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# SatanClaw Agent Persona\n\n"
                "<!-- Edit this file to customize how SatanClaw communicates. -->\n\n"
                "You are SatanClaw, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = satanclaw_home / "memories"
    ...
    state_db_path = satanclaw_home / "state.db"
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/satanclaw-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'satanclaw setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: satanclaw skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: satanclaw memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: satanclaw memory setup")
            issues.append("No Honcho API key — run 'satanclaw memory setup'")
        else:
    ...
    # =========================================================================
    try:
        from satanclaw_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "satanclaw -p" in content:
                            _m = _re.search(r"satanclaw -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'satanclaw doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
