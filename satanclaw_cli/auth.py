"""
Multi-provider authentication system for SatanClaw Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.satanclaw/auth.json with cross-process file locking.
...
from satanclaw_cli.config import get_satanclaw_home, get_config_path
...
DEFAULT_NOUS_PORTAL_URL = "https://portal.eskayML.com"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.eskayML.com/v1"
DEFAULT_NOUS_CLIENT_ID = "satanclaw-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from satanclaw_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `satanclaw model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.satanclaw/auth.json
# =============================================================================
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `satanclaw logout`.
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "No inference provider configured. Run 'satanclaw model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.satanclaw/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.satanclaw/auth.json (not ~/.codex/)
#
# SatanClaw maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from SatanClaw auth store (~/.satanclaw/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `satanclaw login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `satanclaw login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `satanclaw login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `satanclaw login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to SatanClaw auth store (~/.satanclaw/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating SatanClaw auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to SatanClaw auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from SatanClaw's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to SatanClaw auth store")
            print("⚠️  Migrating Codex credentials to SatanClaw's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `satanclaw login` to create a fully independent session.\n")
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other SatanClaw processes
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip SatanClaw models — they're not reliable for agentic tool-calling
            if "satanclaw" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "SatanClaw is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "satanclaw-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("SatanClaw is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `satanclaw status` output."""
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that SatanClaw can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- SatanClaw provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `satanclaw login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from satanclaw_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'satanclaw model' or 'satanclaw setup' instead."""
    print("The 'satanclaw login' command has been removed.")
    print("Use 'satanclaw model' to select a provider and model,")
    print("or 'satanclaw setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.satanclaw/auth.json."""

    # Check for existing SatanClaw-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in SatanClaw auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("SatanClaw will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            ...
            print("SatanClaw will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — SatanClaw gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(SatanClaw creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to SatanClaw auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting SatanClaw login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("SatanClaw will use OpenRouter for inference.")
        else:
            print("Run `satanclaw model` or configure an API key to use SatanClaw.")
    else:
