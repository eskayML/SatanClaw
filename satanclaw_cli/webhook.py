"""satanclaw webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.satanclaw/webhook_subscriptions.json and are
...
def _satanclaw_home() -> Path:
    return Path(
        os.getenv("HERMES_HOME", str(Path.home() / ".satanclaw"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _satanclaw_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: satanclaw gateway run
"""
...
def webhook_command(args):
    """Entry point for 'satanclaw webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: satanclaw webhook {subscribe|list|remove|test}")
        print("Run 'satanclaw webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (satanclaw gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: satanclaw webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from satanclaw webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (satanclaw gateway run)")
