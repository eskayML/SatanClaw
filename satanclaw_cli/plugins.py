"""
SatanClaw Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.satanclaw/plugins/<name>/``
2. **Project plugins** – ``./.satanclaw/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``satanclaw_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "satanclaw_agent.plugins"

_NS_PARENT = "satanclaw_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.satanclaw/plugins/)
        satanclaw_home = os.environ.get("HERMES_HOME", os.path.expanduser("~/.satanclaw"))
        user_dir = Path(satanclaw_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.satanclaw/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".satanclaw" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``satanclaw_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``satanclaw tools`` TUI so plugin-provided toolsets appear
