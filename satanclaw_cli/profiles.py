"""
Profile management for multiple isolated SatanClaw instances.

Each profile is a fully independent HERMES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.satanclaw/profiles/<name>/`` by default.

The "default" profile is ``~/.satanclaw`` itself — backward compatible,
zero migration needed.

Usage::

    satanclaw profile create coder          # fresh profile + bundled skills
    satanclaw profile create coder --clone  # also copy config, .env, SOUL.md
    satanclaw profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    satanclaw -p coder chat                 # or via flag
    satanclaw profile use coder             # set as sticky default
    satanclaw profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.satanclaw) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "satanclaw-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "satanclaw", "default", "test", "tmp", "root", "sudo",
})

# SatanClaw subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.satanclaw/profiles/`` — anchored to the user's home,
    NOT to the current HERMES_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".satanclaw" / "profiles"


def _get_default_satanclaw_home() -> Path:
    """Return the default (pre-profile) HERMES_HOME path."""
    return Path.home() / ".satanclaw"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_satanclaw_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.satanclaw
...
def get_profile_dir(name: str) -> Path:
    ...
    if name == "default":
        return _get_default_satanclaw_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, satanclaw subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a satanclaw subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_satanclaw_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.satanclaw)."
        )
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.satanclaw).\n"
            "To remove everything, use: satanclaw uninstall"
        )
    ...
    # Check for service
    from satanclaw_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from satanclaw_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.satanclaw/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: satanclaw profile create {name}"
        )
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to ``~/.satanclaw``.
    Returns the profile name if HERMES_HOME points into ``~/.satanclaw/profiles/<name>``.
    Returns ``"custom"`` if HERMES_HOME is set to an unrecognized path.
    """
    from satanclaw_constants import get_satanclaw_home
    satanclaw_home = get_satanclaw_home()
    resolved = satanclaw_home.resolve()

    default_resolved = _get_default_satanclaw_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.satanclaw itself — its parent is ~/ and its
        # directory name is ".satanclaw", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: satanclaw profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.satanclaw itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.satanclaw). "
            "Specify a different name: satanclaw profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for satanclaw profile names."""
    return '''# SatanClaw Agent profile completion
# Add to ~/.bashrc: eval "$(satanclaw completion bash)"

_satanclaw_profiles() {
    local profiles_dir="$HOME/.satanclaw/profiles"
...
_satanclaw_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_satanclaw_profiles)" -- "$cur"))
        return
    fi
...
complete -F _satanclaw_completion satanclaw
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for satanclaw profile names."""
    return '''#compdef satanclaw
# SatanClaw Agent profile completion
# Add to ~/.zshrc: eval "$(satanclaw completion zsh)"

_satanclaw() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.satanclaw/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.satanclaw/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any satanclaw modules
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: satanclaw profile create {profile_name}"
        )
