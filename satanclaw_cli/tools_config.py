"""
Unified tool configuration for SatanClaw Agent.

`satanclaw tools` and `satanclaw setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.satanclaw/config.yaml under
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from satanclaw_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "satanclaw-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "satanclaw-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "satanclaw-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "satanclaw-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "satanclaw-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "satanclaw-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "satanclaw-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "satanclaw-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "satanclaw-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "satanclaw-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "satanclaw-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "satanclaw-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "satanclaw-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "satanclaw-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "satanclaw-webhook"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_satanclaw_home()}/satanclaw-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in satanclaw-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "satanclaw-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "satanclaw-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `satanclaw tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by satanclaw tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (satanclaw-cli, satanclaw-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.satanclaw/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `satanclaw tools` and `satanclaw setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ SatanClaw Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from satanclaw_constants import display_satanclaw_home
    print(color(f"  Tool configuration saved to {display_satanclaw_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'satanclaw' or gateway restart.", Colors.DIM))
    print()
