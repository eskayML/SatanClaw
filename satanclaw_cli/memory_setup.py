"""satanclaw memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as satanclaw tools)
# ---------------------------------------------------------------------------
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.satanclaw/plugins/ and try again.\n")
        return
    ...
    env_path = Path(os.environ.get("HERMES_HOME", os.path.expanduser("~/.satanclaw"))) / ".env"
    env_writes = {}
    ...
    satanclaw_home = str(Path(os.environ.get("HERMES_HOME", os.path.expanduser("~/.satanclaw"))))
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, satanclaw_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.satanclaw/plugins/")
