"""Built-in boot-md hook — run ~/.satanclaw/BOOT.md on gateway startup.

This hook is always registered. It silently skips if no BOOT.md exists.
To activate, create ``~/.satanclaw/BOOT.md`` with instructions for the
...
HERMES_HOME = Path(os.environ.get("HERMES_HOME", Path.home() / ".satanclaw"))
