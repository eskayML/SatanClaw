...
``httpx.Auth``) with Satan-specific token storage and browser-based
...
# ---------------------------------------------------------------------------
# Token storage — persists tokens + client info to ~/.satan/mcp-tokens/
# ---------------------------------------------------------------------------
...
class SatanTokenStorage:
    ...
    def _base_dir(self) -> Path:
        home = Path(os.environ.get("HERMES_HOME", Path.home() / ".satan"))
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    connecting and potentially hanging Satan startup indefinitely.
    """
    global _oauth_port
    ...
    if not code:
        raise OAuthNonInteractiveError(
            ...
            "Run 'satan mcp auth <server-name>' to authorize interactively."
        )
    return code, state
...
def build_oauth_auth(server_name: str, server_url: str):
    ...
    storage = SatanTokenStorage(server_name)
    ...
    if not interactive:
        ...
        if not has_cached:
            logger.warning(
                ...
                "connect. Run 'satan mcp auth %s' to authorize interactively.",
    ...
    if not interactive:
        ...
        async def _noninteractive_callback() -> tuple[str, str | None]:
            raise OAuthNonInteractiveError(
                ...
                f"(no TTY). Run 'satan mcp auth {server_name}' to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    SatanTokenStorage(server_name).remove()
