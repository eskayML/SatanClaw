...
``httpx.Auth``) with SatanClaw-specific token storage and browser-based
...
# ---------------------------------------------------------------------------
# Token storage — persists tokens + client info to ~/.satanclaw/mcp-tokens/
# ---------------------------------------------------------------------------
...
class SatanClawTokenStorage:
    ...
    def _base_dir(self) -> Path:
        home = Path(os.environ.get("HERMES_HOME", Path.home() / ".satanclaw"))
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    connecting and potentially hanging SatanClaw startup indefinitely.
    """
    global _oauth_port
    ...
    if not code:
        raise OAuthNonInteractiveError(
            ...
            "Run 'satanclaw mcp auth <server-name>' to authorize interactively."
        )
    return code, state
...
def build_oauth_auth(server_name: str, server_url: str):
    ...
    storage = SatanClawTokenStorage(server_name)
    ...
    if not interactive:
        ...
        if not has_cached:
            logger.warning(
                ...
                "connect. Run 'satanclaw mcp auth %s' to authorize interactively.",
    ...
    if not interactive:
        ...
        async def _noninteractive_callback() -> tuple[str, str | None]:
            raise OAuthNonInteractiveError(
                ...
                f"(no TTY). Run 'satanclaw mcp auth {server_name}' to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    SatanClawTokenStorage(server_name).remove()
