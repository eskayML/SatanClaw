...
def get_stt_model_from_config() -> Optional[str]:
    """Read the STT model name from ~/.satan/config.yaml.
    ...
    try:
        import yaml
        cfg_path = get_satan_home() / "config.yaml"
        if cfg_path.exists():
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="satan-local-stt-") as output_dir:
