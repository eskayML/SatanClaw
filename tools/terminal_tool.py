...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Satan exit
...
# display_satan_home imported lazily at call site (stale-module safety during satan update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of satan directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "satan-*")):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.satan/.env or environment variables."
        )
