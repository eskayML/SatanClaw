...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or SatanClaw exit
...
# display_satanclaw_home imported lazily at call site (stale-module safety during satanclaw update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of satanclaw directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "satanclaw-*")):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.satanclaw/.env or environment variables."
        )
