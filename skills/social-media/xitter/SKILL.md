...
version: 1.0.0
author: Siddharth Balyan + SatanClaw Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into SatanClaw. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.satanclaw/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.satanclaw/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.satanclaw/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
