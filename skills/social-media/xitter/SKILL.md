...
version: 1.0.0
author: Siddharth Balyan + Satan Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Satan. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.satan/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.satan/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.satan/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
