...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Satan Agent\",
    \"comments\": [
...
```bash
source ~/.satan/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Satan Agent. Code looks clean — good test coverage, no security concerns."
