...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.satanclaw/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERMES_HOME", Path.home() / ".satanclaw")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # Nous/SatanClaw models — already uncensored, just needs clean prompt
    "satanclaw": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "satanclaw" in model_lower or "nous" in model_lower:
        return "satanclaw"
...
def _get_current_model() -> tuple:
    """Read current model and provider from SatanClaw config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.satanclaw/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart SatanClaw for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart SatanClaw for changes to take effect.")
